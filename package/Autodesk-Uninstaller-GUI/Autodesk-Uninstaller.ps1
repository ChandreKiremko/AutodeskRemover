...
if ($selectedAppIdentifiers -and $selectedAppIdentifiers.Count -gt 0) {
    ...
    if ($deleteFolderChoice.ToLower() -eq 'yes') {
       ...
       if (Test-Path 'C:\Autodesk') {
           ...
           else { Write-Host "Error deleting C:\Autodesk folder. You may need to do it manually." -ForegroundColor Red }
       } else {
           Write-Host "C:\Autodesk folder not found." -ForegroundColor Yellow
       }
    }

    Write-Host "It is recommended to restart the computer to finalize uninstallation." -ForegroundColor Yellow
    ...
    Write-Host "No products were selected for uninstallation, or the process was cancelled." -ForegroundColor Yellow
}
